	...
	<div class="content">
		...
		<input type="hidden" name="_token" value="{{{ csrf_token() }}}" />
          ...
          <div class="card-body bg-light">
            ...
            <div class="text-center mb-4">
                <h5 style="font-size:18px;">INVENTORY NAME</h5>
            </div>
        
         
            <form id="inventoryNameForm" style="margin: 20px;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                <div class="d-flex justify-content-between mb-3">                 
                    <div style="flex-basis: 48%;">Inventory Name: <input type="text" name="inventory_name" class="form-control" id="inventory_name" /></div> <div style="flex-basis: 48%;">Inventory Description: <input type="text" name="inventory_desc" class="form-control" id="inventory_desc" /></div>
                </div>
...
<script>
 ...
 $(document).ready(function() {
        ...
        $('#requestSubmit').on('click', function(e) {
            ...
            const formData = new FormData(document.getElementById('inventoryNameForm'));
            var btn = $(this);
            var html = $(this).html();

            $.ajax({
                url: "{{ url('pc/create_inventory_name') }}",
                type: "POST",
                data: formData,
                processData: false,  
                contentType: false, 
